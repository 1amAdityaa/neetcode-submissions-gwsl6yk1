class Solution {
    public int maxAreaOfIsland(int[][] grid) {
        ...
        return maxarea; //mistake
        
    }

    private static int dfs(int row,int col,int[][] grid)//char mistake
    {
        ...
        int area=1;
        if(row<0 || col<0 || newrow<=row || newcol<=col || grid[row][col]==0) return 0; //boolean mistake

        grid[row][col]=0; //visited node
        for(int [] dir: directions)
        {
            area+= dfs(row+dir[0],col+dir[1],grid); //full argument passing bro
        }
        return area;
